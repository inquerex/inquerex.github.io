...
* `obtain_metadata_from_filenames_when_possible`: Set this to "y" if you want Inquerex to try to obtain the author(s), the publication year(s), and the title of each search text by reading the PDF file name and looking for special characters that divide the file name into those three bits of metadata (author, year, title). See [the section below](#valid-character-to-divide-file-names) on the use of a special character to mark the division of the file name into three parts; and see [the section after that](#valid-character-to-divide-author-names) on the use of a different special character to mark the division between multiple authors in the first part of the file name.
