...
The other variables, and how to edit/assign values, should be easy enough to interpret by name. Note that the values for `query_name`, `query`, and `pdf_set_name` *must* be enclosed in double quotes if they contain spaces (e.g., `pdf_set_name="Chapters 1-3"`). Double quotes are *optional* for all other variable values. In particular, there's no need to enclose the paths to the PDFs in double quotes. You can, if you want. But Inquerex will do it for you, automatically, if you don't. So don't worry about it. 

One thing you do need to worry about, however, is the presence of double quotation marks in *any* variable value. For example, if you wish to use a value for `pdf_set_name` that contains quotation marks (e.g., `pdf_set_name="Articles on So-and-So's Use of the Words "X" and "Y" in Different Contexts"`), then you'll need to type a backslash (i.e., `\`) before each quotation mark. This is called "escaping" a character (in this case, you're escaping the `"` character so that Inquerex doesn't interpret it as the end of the variable value). The same applies to quotation marks in paths to PDFs. If any folder names or file names contain double quotation marks, you'll need to "escape" them by typing `\` before each of them. So in the example of the `pdf_set_name` value, above, you would need to escape the quotation marks like so: `pdf_set_name="Articles on So-and-So's Use of the Words \"X\" and \"Y\" in Different Contexts"`.

Once you enter the values you want in the program file, save it, and move it to the Active Programs folder in [your Inquerex folder](#your-inquerex-folder). Whenever you launch Inquerex, if it finds one or more `.txt` files in the Active Programs folder, it'll try to perform a series of separate searches using the variable values from each program file, one program file at a time. Imagine that you have a series of searches you want to perform on a large number of pdf files. You could write a program file for each search, put all the program files into the Active Programs directory, launch the app, and then run them overnight.
