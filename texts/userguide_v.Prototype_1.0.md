...
>Note that there's no need to enclose variable values in double quotation marks. You can do it, if you want. But Inquerex will do it for you, automatically, whenever it loads a program file. Conversely, Inquerex will [escape](https://en.wikipedia.org/wiki/Escape_character) double quotation marks that appear in your variable values, unless they're the very first or the very last character (in which case, Inquerex will interept the quotation marks as computer code, marking the beginning/end of the variable value, rather than as a part of the variable value itself).
