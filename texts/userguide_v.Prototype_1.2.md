...
Some of the [libraries above](#direct-function-calls) make function calls, themselves, to a handful of *other* libraries. To see these other libraries in Finder you'll need to make it possible to see "hidden files." To do that, click on the Finder app, then press `command`, `shift`, and `.` all at the same time. Pressing this combination of keys all at the same time toggles the "view hidden files" setting on and off. Then you can find these libraries in `/usr/local/opt` on the relevant disk (e.g., "Macintosh HD"). That's where Inquerex installs them for you.
